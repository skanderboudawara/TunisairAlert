...
def json_location(type_flight, datetime_query):
    ...
    file_flight_type = f'{datetime_query.strftime("%d-%m-%Y")}_{type_flight}_flights.json'
    return f'{path_flight_type}/{file_flight_type}'


def correct_datetime_info(datetime_actual, datetime_estimated, datetime_scheduled, flight_status, datetime_delay, text):
    '''
    ...
    datetime_datetime_scheduled = datetime.fromisoformat(datetime_scheduled).astimezone(pytz.timezone(tz))
    effective_date = datetime_datetime_scheduled
    flight_status = flight_status
    datetime_delay = datetime_delay if (datetime_delay is not None) & (datetime_delay != '') else 0

    for date_check in [datetime_estimated, datetime_actual]:
        if (date_check != '') & (date_check is not None):
            effective_date = datetime.fromisoformat(date_check).astimezone(pytz.timezone(tz))
