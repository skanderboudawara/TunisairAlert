...
def plotFromToAirport(current_time):
    ...
    def create_plot(type_flight, from_airport, to_airport):
        ...
        df = df[(df['ARRIVAL_COUNTRY'] == to_airport) &
        ...
        df_ftype_delay = df_ftype_delay.fillna(0).replace('', 0).replace('TU', 'TUNISAIR').replace(
            'AF', 'AIR FRANCE').replace('BJ', 'NOUVELAIR').replace('HV', 'TRANSAVIA')
        ...
        ax.set_title(f'AVG delays from {from_airport[:3]} to {to_airport[:3]}',
        ...
        picture_to_save = get_pic_location(
            current_time, 'delayreport', type_flight)
        fig.savefig(picture_to_save, bbox_inches='tight')
        return picture_to_save
    return create_plot('ARRIVAL', 'FRANCE', 'TUNISIA'), create_plot('DEPARTURE', 'TUNISIA', 'FRANCE')
...
def plotArrDepDelay(current_time):
    ...
    font_prop = get_font_prop('LEDBDREV.TTF')
    # Title
    ax.set_title('AVG delays of TUNISAIR',
                 fontproperties=font_prop, y=1.2)
    ax.title.set_fontsize(12)
    ax.title.set_color('orange')
